(ns dana_indonesia.main
  "Kotodama WASM entrypoint for the dana_indonesia clean-room actor (L4) — Clojure port.
  ...
  facts (`dana_indonesia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dana_indonesia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dana_indonesia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dana_indonesia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dana_indonesia-compat/ready)
